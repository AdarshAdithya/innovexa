...
## 6. Agent (`app/agent.py::SYSTEM`)
Four tools and the decision logic in the system prompt. We do not trust the agent's decision:
it is always re-checked against `decide()`.
